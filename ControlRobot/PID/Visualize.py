...
setpoint_arah = 0
setpoint_jarak = 0
measurement_jarak = 30  # Jarak awal
frame = np.zeros((400, 600, 3), dtype=np.uint8)

def nothing(x):
    pass

# Membuat jendela dan trackbar
...
# Loop utama simulasi
while True:
    # Ambil nilai dari trackbar dan konversi ke error arah
    slider_val = cv2.getTrackbarPos("Error Arah", "Simulasi PID")
    measurement_arah = slider_val - 50

    # Update parameter PID dari trackbar
    ...
    # Visualisasi ke jendela
    frame[:] = (0, 0, 0)  # Bersihkan frame
    ...
    # Keluar jika tekan ESC
    if cv2.waitKey(1) == 27:
        print("ESC ditekan, keluar dari simulasi.")
        break

    # Update simulasi jarak
